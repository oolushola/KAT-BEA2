...
@section('title')Kaya ::. BUH Target @stop

@section('main')
<div class="page-header page-header-light">
    <div class="page-header-content header-elements-md-inline">
        <div class="page-title d-flex">
            <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Home - BUH</span> - Targets</h4>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form method="POST" name="frmTarget" id="frmTarget">
                    ...
                    <div class="form-group">
                        <label>B.U.H Target</label>
                        <select class="form-control" name="user_id" id="userId">
                            <option value="">Choose a unit head</option>
                            @foreach($buhs as $buh)
                            @if(isset($recid) && $recid->user_id == $buh->id)
                            <option value="{{$buh->id}}" selected>{{$buh->first_name}} {{$buh->last_name}}</option>
                            @else
                            <option value="{{$buh->id}}">{{$buh->first_name}} {{$buh->last_name}}</option>
                            @endif
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" placeholder="100" name="target" id="monthlyTarget" value="<?php if(isset($recid)) { echo strtoupper($recid->target); } ?>">
                    </div>

                    <div class="text-right">
                        <span id="loader"></span>
                        @if(isset($recid))
                        <button type="submit" class="btn btn-primary" id="updateMonthlyBuhTarget">Update 
                        @else
                        <button type="submit" class="btn btn-primary" id="addMonthlyBuhTarget">Add  
                        @endif Target
                            ...
                            <tr class="{{$css}}" style="font-size:10px">
                                ...
                                <td>
                                    ...
                                    <div class="list-icons">
                                        <a href="{{URL('buh-target/'.$target->id.'/edit')}}" class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
